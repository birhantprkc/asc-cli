...
- **`docs/commands.md`**: generated by `make docs` from `asc --experimental-dump-help` (or the `generate-manual` plugin). It is the only complete flag list. `skills/asc-cli/references/commands.md` links to it instead of copying it.
- **`docs/features/<x>/<topic>.md`**: deep dives that are too long for the feature doc, such as IAP pricing or the iris SRP flow.
