...
```bash
# 1. Upload a build and wait for processing                         → features/builds-archive, builds-upload
...
# 2. Distribute to TestFlight                                        → features/testflight
...
# 4. Update What's New                                               → features/version-localizations
...
# 5. Pre-flight check, then submit                                   → features/version-check-readiness
asc versions check-readiness --version-id "$VERSION_ID" --pretty
asc versions submit --version-id "$VERSION_ID"
```

**First-time in-app purchases or subscriptions?** Apple requires them to go to review with an app version. Add `--with-products` to step 5 (try `--dry-run` first): see [submit with products](features/submit-with-products/README.md).

**Rejected?** Find the rejected item with [review submissions](features/review-submissions/README.md), then read App Review's message with [resolution center](features/resolution-center/README.md).
