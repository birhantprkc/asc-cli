...
|-------|----------|-------------|
| `developer` | yes | Display handle shown on the card (`@developer`) |
...
- `Developer X is already listed`: an entry with the same `developer` is already in `apps.json`; check the existing PR.
- `Timed out waiting for fork`: the fork took more than 24 seconds; retry after a moment.
